...
El contraste de esfericidad de Bartlett permite comprobar si las correlaciones son distintas de 0 de modo significativo. La hip?tesis nula es que det(R)=1. La función "cortest.bartlett" del paquete "pysch" realiza este test. Esta función trabaja con datos normalizados.
...
mean(PCA$sdev^2)
```
En este caso la media es 1, luego se escogerían la primera, segunda y tercera. Esto son, 3 componentes principales. ¿CUANTA VARIANZA EXPLICADA ACUMULAN? TODO
...
paste("trainig_error=", trainig_error, "%")
```
El error con los datos de training es 
...
CREO QUE YA ESTAN NORMALIZADOS Y ESTO NO HACE FALTA TODO
